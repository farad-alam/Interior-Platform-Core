...
export function BrandMarquee({ brands, isAr }: BrandMarqueeProps) {
  ...
  useAnimationFrame((t, delta) => {
    let moveBy = baseVelocity * (delta / 1000) * 50 // Adjust speed by changing 50
      ...
      <div className="relative w-full max-w-7xl mx-auto flex overflow-hidden group">
        ...
        >
          {loopedBrands.map((brand, idx) => (
            <div 
              key={`${brand.id}-${idx}`} 
              className="relative w-[140px] md:w-[180px] h-[70px] md:h-[90px] shrink-0 opacity-60 hover:opacity-100 grayscale hover:grayscale-0 transition-all duration-500 hover:scale-105 cursor-pointer"
            >
              {brand.url ? (
